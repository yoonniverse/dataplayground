...
1. Divide the input image to 3x3 grid (in practice, we use finer grid like 19x19).
2. Label of an image would be of shape 3x3x8 since each grid will be a vector of $[p_c, b_x, b_y, b_h, b_w, c_1, c_2, c_3]$.<br>
If the center of the detected car is in the middle-left grid like in the example above, middle-left vector of the label would be like this; $[1, 0.6, 0.5, 0.7, 0.6, 0, 1, 0]$. $p_c$ of the top-right part will be 0, since there is no obect there.<br>
...
3. Train a convolutional network that yields output with shape 3x3x8.
...
In the above example, red box is label and purple box is prediction. IoU equals size of yellow box divide by size of green box. Here, we've set threshold to 0.5; consider it predicted correct if IoU is bigger than 0.5. We can make this threshold bigger if we want to be more precise.
...
As shown above, when grid is fine, many grids can predict that they've detected an object which is actually the same one.
...
1. Discard parts where $p_c<0.6$(you can change value 0.6).
